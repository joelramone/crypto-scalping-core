"""EMA Pullback research strategy."""
...
STOP_LOSS_PCT = 0.002
MAX_HOLDING_CANDLES = 35


class EmaPullbackStrategy(BaseStrategy):
    """Long-only EMA pullback strategy for research backtests."""

    def generate_entries(self, df: pd.DataFrame) -> pd.Series:
        """Return EMA Pullback long-only entry signals."""
        strategy_df = df.copy()
        ...
        return (
            ...
            & (strategy_df["ema20"] > strategy_df["ema50"])
            & (strategy_df["ema50"] > strategy_df["ema200"])
            & (strategy_df["ema20_slope"] > 0.0)
            & (strategy_df["close"] <= strategy_df["ema20"])
            & (strategy_df["close"] >= strategy_df["ema50"])
            & (strategy_df["rsi14"] >= 35.0)
            & (strategy_df["rsi14"] <= 55.0)
            & (strategy_df["volume_ratio"] > 0.8)
            & (strategy_df["atr14"] > strategy_df["atr14_median"])
        )

    def generate_exits(self, df: pd.DataFrame) -> pd.Series:
        """Return no indicator-specific exits for EMA Pullback."""
