"""Pre-registered Momentum Pullback / Trend Continuation baseline."""
...
class MomentumPullbackContinuationParameters(BaseModel):
    """The baseline's frozen simulator exit parameters."""
...
class MomentumPullbackContinuationStrategy(BaseStrategy):
    """Enter long when an EMA-aligned pullback confirms continuation."""
    ...
    def __init__(
        ...
        self.parameters = MomentumPullbackContinuationParameters(
    ...
    def generate_entries(self, df: pd.DataFrame) -> pd.Series:
        """Return the exact causal trend, pullback, and confirmation signal."""
    ...
    def name(self) -> str:
        return "momentum_pullback_continuation"
