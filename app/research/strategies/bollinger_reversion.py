"""Bollinger Reversion research strategy."""
...
class BollingerReversionStrategy(BaseStrategy):
    """Long-only Bollinger Bands mean reversion strategy for research backtests."""

    def generate_entries(self, df: pd.DataFrame) -> pd.Series:
        """Return Bollinger Reversion long-only entry signals."""
        strategy_df = df.copy()
        ...
        return (
            (strategy_df["close"] < strategy_df["bb_lower"])
            & (strategy_df["rsi14"] < 35.0)
            & (strategy_df["volume_ratio"] > 0.7)
            & (strategy_df["atr14"] > strategy_df["atr14_median"])
        )

    def generate_exits(self, df: pd.DataFrame) -> pd.Series:
        """Return no indicator-specific exits for Bollinger Reversion."""
